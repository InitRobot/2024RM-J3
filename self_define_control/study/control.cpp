...
class p_data
{   //定义键值类，数据成员有鼠标键值、鼠标移动、包序号、按键数、按键值数组（闲哥出品，必属精品）
    public:
    String  str_raw;
    int cmd_id, len, mouse_press, mouse_x, mouse_y, seq, key_num, key_vals[20], values[20];
    p_data(String str_init)
    {//初始化函数获取SDK字符串并解析成各个数据
        str_raw = String(str_init);
        // Serial.println("ok?"+String(str_init));
        if (str_init.startsWith("game") && str_init.indexOf('game', 5) == -1)
        {//长度够长的字符串才被认为是键值数据
